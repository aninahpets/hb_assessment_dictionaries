...
def word_length_sorted(words):
    ...
    for word in words:
        count = len(word)
        if count in lengths_and_words:
            lengths_and_words[count].append(word)
        else:
            lengths_and_words[count] = [word]
    return lengths_and_words.items()
...
def translate_to_pirate_talk(phrase):
    ...
    phrase = phrase.split()
    for word in phrase:
        if len(pirate_phrase) > 1:
                pirate_phrase += " " + pirate_translations.get(word, word)
        else:
            pirate_phrase = pirate_translations.get(word, word)
